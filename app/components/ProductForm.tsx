...
export default function ProductForm(props: Props) {
    ...
    const removeImage = async (index: number) => {
        //if the image is from the cloudinary
        //then we need to remove it from the cloudinary
        if (!productImagesSource) return;

        const imageToRemove = productImagesSource[index];

        const cloundSourceUrl = 'https://res.cloudinary.com'

        if (imageToRemove.startsWith(cloundSourceUrl)) {
