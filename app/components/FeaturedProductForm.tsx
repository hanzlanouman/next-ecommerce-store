...
export default function FeaturedProductForm({ initialValue }: Props) {
    ...
    const handleUpdate = async () => {
        ...
        try {
            const { file, link, linkTitle, title } = await oldFeaturedProductValidationSchema.validate({ ...featuredProduct }, {
                abortEarly: false
            });
            ...
            if (file) {
                ...
                const publicId = await extractPublicId(initialValue.banner);
    ...
    const handleCreate = async () => {
        try {
            const { file, link, linkTitle, title } = await newFeaturedProductValidationSchema.validate({ ...featuredProduct }, {
                abortEarly: false
            });
    ...
    return (
        <form
                ...
                <div className="h-[380px] w-full flex flex-col items-center justify-center border border-dashed border-blue-gray-400 rounded cursor-pointer relative">
                    {poster ? (
                        <Image
                            ...
                            alt="banner" src={poster || initialValue?.banner} fill />
                    ) : (
                        <>
            ...
            <div className="text-right">
                <Button
                    disabled={isPending}
                    type="submit">{isForUpdate ? "Update" : "Submit"}</Button>
            </div>
        </form>
    );
}
